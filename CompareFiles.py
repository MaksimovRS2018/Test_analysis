        ...
        if list_name_FilesOrDir[name_FileOrDir].find(".cfg") != -1:
            ...
            with open(name_log_file, "a", encoding='utf-8') as f:
                try:
                    ...
                    copyfile(path_oscill_terminal_SUBA + "\\" + list_name_files_oscill_terminal_SUBA[0],
                             path_dst + test + " Terminal SUBA " + list_name_files_oscill_terminal_SUBA[0])
                    copyfile(path_oscill_terminal_SUBA + "\\" + list_name_files_oscill_terminal_SUBA[1],
                             path_dst + test + " Terminal SUBA " + list_name_files_oscill_terminal_SUBA[1])
                ...
                try:
                    ...
                    copyfile(path_oscill_terminal_SUBB + "\\" + list_name_files_oscill_terminal_SUBB[0],
                             path_dst + test + " Terminal SUBB " + list_name_files_oscill_terminal_SUBB[0])
                    copyfile(path_oscill_terminal_SUBB + "\\" + list_name_files_oscill_terminal_SUBB[1],
                             path_dst + test + " Terminal SUBB " + list_name_files_oscill_terminal_SUBB[1])
                    # f.write( "Есть осциллогаммы терминала для опыта " + number_test + " c пуском №" + number_start + ". Имя файла осциллограммы RTDS: " + test + "\n")
                except FileNotFoundError:
                    f.write(
